...
초기 버전에서는 GitHub/GCP 계정 사전조건을 배포 전 수동 조건으로 두고, GKE node의 image pull IAM처럼 클러스터 리소스 경계에 가까운 항목만 Terraform에 포함했다. 이후 반복 가능한 GCP-side bootstrap prerequisite를 Terraform 코드로 표현하고, 기존 수동 생성 리소스를 import해 recreate 없이 관리 대상으로 편입했다. post-import `terraform plan`은 `No changes.`로 확인됐다.

| 항목 | 현재 기준 |
|---|---|
| GCP API enablement | `project_services` module의 `google_project_service`로 관리. 기존 활성화 API 8개 import 완료 |
| GitHub OIDC/WIF | `github_wif` module로 deploy service account, WIF pool/provider, impersonation binding 관리. 기존 수동 리소스 5개 import 완료 |
...
`serviceusage.googleapis.com` 자체도 Terraform 관리 대상에 포함하지만, 첫 bootstrap 시 Google provider가 Service Usage API에 접근할 수 있어야 한다. 현재 프로젝트는 수동 활성화, import, post-import plan 안정화가 완료됐다. 새 프로젝트에서 같은 흐름을 재현할 때는 Service Usage API 접근 가능 상태를 먼저 만든 뒤 기존 활성화 API를 import한다.
...
- 기존에 수동 활성화된 API는 `terraform import`로 state에 편입했고 post-import plan을 확인했다. 새 수동 API가 추가되면 같은 방식으로 state에 편입한 뒤 plan을 확인한다.
...
현재 API enablement와 GitHub Actions WIF prerequisite는 수동 생성 및 검증 이후 Terraform 코드와 state에 편입됐다. 이 섹션은 같은 흐름을 새 환경에서 재현할 때 중복 생성 또는 IAM drift 없이 기존 리소스를 state에 편입하기 위한 import pattern을 남긴다.
...
| API enablement | `project_services` module로 Terraform 관리 | 코드 추가, 기존 8개 API import 완료. post-import `terraform plan` 결과 `No changes.` 확인 |
| GitHub OIDC/WIF | `github_wif` module로 GCP-side prerequisite Terraform 관리. GitHub secrets는 수동 유지 | 코드 추가, deploy service account/writer IAM/WIF pool·provider/workloadIdentityUser binding import 완료. post-import `terraform plan` 결과 `No changes.` 확인 |
