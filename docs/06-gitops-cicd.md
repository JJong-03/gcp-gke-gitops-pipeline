...
Terraform output으로 같은 식별자를 확인할 수 있더라도, GitHub secret 등록과 secret 값 관리는 이번 단계에서 Terraform state 밖에 둔다.
...
2026-04-19 기준 deploy service account 생성, Artifact Registry repository-scoped writer 권한 부여, Workload Identity Pool/Provider 생성, repository-scoped `roles/iam.workloadIdentityUser` binding까지 완료됐다. Provider는 실제 GitHub repository 조건으로 제한되어 있다. 2026-04-19 현재 Terraform 코드에는 같은 리소스 정의가 추가됐으며, 기존 수동 리소스를 import한 뒤 `terraform plan`이 안정적인지 확인해야 한다.
...
| 항목 | 상태 |
|---|---|
| Workload Identity Pool/Provider | 수동 구성 완료, Terraform 코드 추가, import 대기 |
| deploy service account | 수동 생성 완료, Terraform 코드 추가, import 대기 |
| Artifact Registry writer IAM | 수동 구성 완료, Terraform 코드 추가, import 대기 |
| `roles/iam.workloadIdentityUser` binding | 수동 구성 완료, Terraform 코드 추가, import 대기 |
...
| 주체 | 권한 | 범위 | 관리 방식 |
|---|---|---|---|
| GitHub Actions deploy service account | `roles/artifactregistry.writer` | repository scope | 수동 구성 및 검증 완료, Terraform import 대기 |
...
| Google Cloud 인증 | GitHub OIDC + Workload Identity Federation 수동 구성 및 검증 완료. GCP-side prerequisite Terraform 코드 추가, import/plan 검토 필요 |
