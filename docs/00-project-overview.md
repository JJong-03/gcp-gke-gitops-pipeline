...
| 문서 구조 | `docs/00`부터 `docs/10`까지 파일 구조 고정 |
| Terraform | `network`, `gke`, `artifact_registry`, `project_services`, `github_wif` 모듈 구현 완료. apply와 기존 수동 리소스 import, post-import plan `No changes.` 확인 완료. |
...
| 항목 | 초기 기준 |
|---|---|
| GCP API enablement | Terraform 관리 대상으로 전환 완료. `disable_on_destroy = false`로 destroy 시 API 자체는 비활성화하지 않음 |
| GitHub Actions 인증 | GitHub OIDC와 Workload Identity Federation 구성 완료. GCP-side prerequisite는 Terraform 관리 대상으로 전환 완료, GitHub secrets는 수동 유지 |
...
| Runbook | 처음부터 다시 따라 실행할 수 있는 [Reproduction Runbook](10-reproduction-runbook.md) |
