...
| Terraform | `network`, `gke`, `artifact_registry` 모듈 구현 완료. node locations, GKE node service account IAM, node disk size 명시적 설정 반영. `terraform init/validate/plan/apply` 완료. |
...
- GitHub OIDC/Workload Identity Federation 구성의 Terraform 자동화
- GCP API enablement의 Terraform 자동화
...
| 항목 | 초기 기준 |
|---|---|
| GCP API enablement | 필요한 API를 사전에 수동 활성화 |
| GitHub Actions 인증 | GitHub OIDC와 Workload Identity Federation을 사전에 수동 구성 |
...
| 구분 | 산출물 |
|---|---|
| Infrastructure | Terraform root module과 `network`, `gke`, `artifact_registry` 하위 모듈 |
