...
초기 버전에서는 GitHub OIDC와 Workload Identity Federation을 수동 구성했고, `main` branch push 기반 Artifact Registry image push까지 검증했다. Terraform으로 GitHub OIDC provider, deploy service account, IAM binding을 자동화하는 작업은 후순위로 둔다.
...
| GitHub OIDC/WIF는 초기 수동 구성으로 검증 | 인증 자동화보다 end-to-end 흐름 검증을 먼저 완료하기 위해 |
