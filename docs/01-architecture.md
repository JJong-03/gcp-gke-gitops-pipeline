...
초기 버전의 Ingress 검증은 domain 없이 진행한다. `host` rule은 제거하고, GKE Ingress가 할당한 External IP로 HTTP 응답을 확인한다.
...
초기 버전에서는 GitHub OIDC와 Workload Identity Federation을 사전에 수동 구성한다. Terraform으로 GitHub OIDC provider, deploy service account, IAM binding을 자동화하는 작업은 후순위로 둔다.
...
Argo CD는 CD와 desired state sync를 담당한다. 현재 `repoURL`은 `https://github.com/OWNER/REPOSITORY.git` placeholder이므로 실제 repository URL로 교체해야 한다.

초기 버전의 image tag 반영은 수동 manifest 갱신 방식이다. GitHub Actions가 Artifact Registry에 push한 image URI를 `k8s/deployment.yaml`에 반영하고, 이후 Argo CD가 Git desired state를 sync한다. 자동 image update는 후순위 개선 사항이다.
...
| GitHub OIDC/WIF는 초기 수동 사전조건으로 유지 | 인증 자동화보다 end-to-end 흐름 검증을 먼저 완료하기 위해 |
...
- Argo CD 설치/bootstrap 절차는 아직 검증되지 않았다.
