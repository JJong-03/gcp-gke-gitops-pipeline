...
초기 버전에서는 GitHub OIDC와 Workload Identity Federation을 수동 구성했고, `main` branch push 기반 Artifact Registry image push까지 검증했다. 이후 수동 구성된 GCP-side 인증 prerequisite를 Terraform 코드로 표현하고 기존 리소스를 import해 관리 대상으로 편입했다. post-import `terraform plan`은 `No changes.`로 확인됐다. GitHub repository secrets 값은 Terraform state에 넣지 않고 수동 prerequisite로 유지한다.
...
Terraform은 Kubernetes workload manifest, Argo CD 설치/bootstrap, Argo CD sync, GitHub Actions workflow 실행 결과를 직접 관리하지 않는다.
...
| GitHub OIDC/WIF는 초기 수동 구성 후 Terraform import로 편입 완료 | end-to-end 검증을 먼저 완료한 뒤, recreate 없이 GCP-side bootstrap prerequisite만 Terraform 관리로 옮기기 위해 |
...
- GCP API enablement와 GitHub Actions WIF prerequisite는 Terraform 코드와 state에 편입됐고, 기존 수동 리소스 13개 import 및 post-import `terraform plan` `No changes.` 확인까지 완료됐다. GitHub secrets는 Terraform state 밖에서 수동 유지하고, Argo CD 설치/bootstrap은 Terraform 범위 밖의 `docs/10-reproduction-runbook.md` 절차로 다룬다.
